...
unsigned int RotWord(unsigned int w) ////ham con trong ham g(w)
{
...
unsigned int SubWord(unsigned int w) //ham con trong ham g(w)
{
	int S[] = {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76,
		...
		0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0xB42, 0x68, 0x41, 0x99, 0x2D, 0x0F, 0xB0, 0x54, 0xBB, 0x16
};
unsigned int kq = 0;
for (int i = 1; i <= 4; i++)
	{
...
unsigned int G(unsigned int w, int j) // bao gom 3 ham con tren
{
...
unsigned int NhanCot(unsigned int w)
{
	...
	unsigned int kq4 = byte1 ^ byte2 ^ byte3 ^ Nhan2(byte4);
...
int main()
{
	unsigned int w0 = 0x2b7e1516, w1 = 0x26aed2a6, w2 = 0xabf71588, w3 = 0x09cf4f3c;
